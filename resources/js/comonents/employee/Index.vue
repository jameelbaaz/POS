                  ...
                  <table class="table align-items-center table-flush">
                    ...
                    <tbody>
                      <tr v-for="employee in employees.data" :key="employee.id">
                        ...
                        <td><img :src="employee.photo" alt="employee" class="em_photo"></td>
                        <td>{{employee.salary}}</td>
                        <td>{{employee.joining_date}}</td>
                        <td>
                          <router-link  :to="{name:'edit-employee',params:{id:employee.id}}" class="btn btn-sm btn-primary text-white">Edit</router-link>
...
getResults(page=1) { 
        ...
        axios.get(`/api/employee?page=${page}&search=${this.search}`) 
        .then(response => { this.employees = response.data; 
